...
class WP_Ru_Max_Contacts {
    ...
    private function channel_link( $item, $type ) {
        ...
        if ( '' === $value ) return '#';
        if ( 'email' === $type ) return 'mailto:' . sanitize_email( $value );
        if ( 'phone' === $type ) return 'tel:' . preg_replace( '/[^0-9+]/', '', $value );
        if ( false === strpos( $value, '://' ) && false !== strpos( $value, '@' ) ) return 'https://t.me/' . ltrim( $value, '@' );
